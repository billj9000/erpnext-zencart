...
@frappe.whitelist()
def updateItemQuantity( store_name, items ):
...
#	settings = frappe.get_doc( "Zen Cart Settings" )
	store_list = frappe.get_all( "Zen Cart Store" )
	for s in store_list:
		logger = frappe.logger("zencart", allow_site=True, file_count=50)
		logger.debug(f"Scheduler task")
		store = frappe.get_doc( "Zen Cart Settings", s['name'] )
		...
		for item_code in items:
			...
			if website_item.published:
				...
				if item.customs_tariff_number:
					hs_code = item.customs_tariff_number.name
				else:
					hs_code = ''
